...
// ============================================
// UPDATED: exportMembersWithBirthdays - Full export with all formats
// ============================================
function exportMembersWithBirthdays(format) {
    ...
    if (format === 'excel') {
        try {
            ...
            showNotification(`${data.length} members exported to Excel!`, 'success');
        } catch (e) {
        ...
        if (monthFilter && monthFilter !== 'all') {
            const monthNames = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
            text += `🎂 Birthday: ${monthNames[parseInt(monthFilter) - 1]}`;
            if (dayFilter && dayFilter !== 'all') {
                text += ` ${parseInt(dayFilter)}`;
            }
            text += '\n';
        }
        text += `${'─'.repeat(50)}\n\n`;
        ...
        data.slice(0, 50).forEach(row => {
            ...
            if (row['Visit Count'] === 1) text += ' ⭐';
            else if (row['Visit Count'] === 2) text += ' 🌟';
            text += '\n';
        });
        if (data.length > 50) {
        ...
        copyToClipboard(text, 'Members list copied to clipboard!');
    } else {
        // HTML export (PDF/Word)
        let title = 'Members List';
        if (monthFilter && monthFilter !== 'all') {
            const monthNames = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
            title += ` - Birthday: ${monthNames[parseInt(monthFilter) - 1]}`;
            if (dayFilter && dayFilter !== 'all') {
                title += ` ${parseInt(dayFilter)}`;
            }
        }
        
        let html = generateExportHTML(data, title, 'Members');
        const ext = format === 'pdf' ? 'html' : 'doc';
        const mime = format === 'pdf' ? 'text/html' : 'application/msword';
        downloadFile(html, mime, `${fileNamePrefix}_${dateStr}.${ext}`);
        showNotification(`${data.length} members exported!`, 'success');
    }
}
...
// ============================================
window.exportMembers = exportMembersWithBirthdays;

// ============================================
// EXPOSE TO GLOBAL SCOPE
// ============================================
window.exportMembersWithBirthdays = exportMembersWithBirthdays;
window.loadMembers = loadMembers;
window.loadFirstTimers = loadFirstTimers;
window.loadSecondTimers = loadSecondTimers;
window.loadAbsentees = loadAbsentees;
window.loadTodayAttendance = loadTodayAttendance;
window.openMemberModal = openMemberModal;
window.saveMemberEdit = saveMemberEdit;
window.deleteMember = deleteMember;
window.closeModal = closeModal;
window.toggleEditWorker = toggleEditWorker;